...
class MarkingVisitorImpl {
 ...
 protected:
  ...
  inline void registerDelayedMarkNoTracing(void** objectPointer) {
    ...
    toDerived()->heap().pushPostMarkingCallback(
        reinterpret_cast<void*>(objectPointer), &markBackingNoTracing);
  }
  ...
  inline void registerMovingObjectReference(void** reference) {
    if (toDerived()->getMarkingMode() != Visitor::GlobalMarkCompacting)
      return;
    toDerived()->heap().registerMovingObjectReference(reference);
  }

  inline void registerMovingObjectCallback(void* backingStore,
                                           void* data,
                                           MovingObjectCallback callback) {
    if (toDerived()->getMarkingMode() != Visitor::GlobalMarkCompacting)
      return;
    toDerived()->heap().registerMovingObjectCallback(backingStore,
                                                     data, callback);
  }
 ...
 private:
  ...
  static void markBackingNoTracing(Visitor* visitor, void* object) {
