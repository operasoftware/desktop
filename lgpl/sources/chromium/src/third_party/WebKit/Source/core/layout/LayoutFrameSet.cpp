...
LayoutFrameSet::LayoutFrameSet(HTMLFrameSetElement* frameSet)
    : LayoutBox(frameSet), m_isResizing(false), m_isChildResizing(false) {
  setInline(false);
}

LayoutFrameSet::~LayoutFrameSet() {}

LayoutFrameSet::GridAxis::GridAxis() : m_splitBeingResized(noSplit) {}
...
void LayoutFrameSet::layOutAxis(GridAxis& axis,
  ...
  const Page* page = frameView()->page();
  float altDpiScale = page ? page->altDeviceScaleFactor() : 1.0f;
  ...
  for (int i = 0; i < gridLen; ++i) {
    ...
    if (grid[i].isAbsolute()) {
      gridLayout[i] =
          max<int>(grid[i].value() * effectiveZoom * altDpiScale, 0);
  ...
  if (countRelative) {
    ...
    for (int i = 0; i < gridLen; ++i) {
      if (grid[i].isRelative()) {
        gridLayout[i] =
            (max(grid[i].value(), 1.) * remainingRelative) / totalRelative;
        remainingLen -= gridLayout[i];
        lastRelative = i;
      }
    }
...
bool LayoutFrameSet::userResize(MouseEvent* evt) {
  if (!m_isResizing) {
    ...
    if (evt->type() == EventTypeNames::mousedown &&
      ...
      FloatPoint localPos =
          absoluteToLocal(FloatPoint(evt->absoluteLocation()), UseTransforms);
      startResizing(m_cols, localPos.x());
      startResizing(m_rows, localPos.y());
      if (m_cols.m_splitBeingResized != noSplit ||
          m_rows.m_splitBeingResized != noSplit) {
        setIsResizing(true);
        return true;
      }
    }
