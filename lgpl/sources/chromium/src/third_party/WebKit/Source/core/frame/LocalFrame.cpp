...
class DragImageBuilder {
 ...
 public:
  DragImageBuilder(const LocalFrame& localFrame,
                   const FloatRect& bounds,
                   float altDeviceScaleFactor)
      : m_localFrame(&localFrame),
        m_bounds(bounds),
        m_altDeviceScaleFactor(altDeviceScaleFactor) {
  ...
  std::unique_ptr<DragImage> createImage(
    ...
    return DragImage::create(image.get(), imageOrientation,
                             screenDeviceScaleFactor * m_altDeviceScaleFactor,
...
class DraggedNodeImageBuilder {
 ...
 public:
  DraggedNodeImageBuilder(const LocalFrame& localFrame,
                          Node& node,
                          float altDeviceScaleFactor)
      : m_localFrame(&localFrame),
        m_node(&node)
...
bool LocalFrame::isSpatialNavigationFindingFocusCandidateEnabled() {
  return m_spatNavFindingFocusCandidateEnabled;
}

void LocalFrame::setSpatialNavigationFindingFocusCandidateEnabled(
    bool spatNavFindingFocusCandidateEnabled) {
  m_spatNavFindingFocusCandidateEnabled = spatNavFindingFocusCandidateEnabled;
}
...
inline LocalFrame::LocalFrame(FrameLoaderClient* client,
    ...
    : Frame(client, host, owner),
      ...
      m_spatNavFindingFocusCandidateEnabled(false),
