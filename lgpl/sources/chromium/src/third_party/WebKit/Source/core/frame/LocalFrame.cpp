...
static std::unique_ptr<DragImage> CreateDragImage(
  ...
  return DragImage::Create(image.Get(), image_orientation,
                           screen_device_scale_factor * alt_device_scale_factor,
                           kInterpolationHigh, opacity);
}
...
class DraggedNodeImageBuilder {
 ...
 public:
  DraggedNodeImageBuilder(const LocalFrame& local_frame, Node& node, float alt_device_scale_factor)
...
#endif
        , alt_device_scale_factor_(alt_device_scale_factor) {
  ...
  std::unique_ptr<DragImage> CreateImage() {
    ...
    return CreateDragImage(
        *local_frame_, 1.0f,
        ...
        bounding_box, alt_device_scale_factor_, builder, border_box_properties);
  }
...
bool LocalFrame::IsSpatialNavigationFindingFocusCandidateEnabled() {
  return spat_nav_finding_focus_candidate_enabled_;
}

void LocalFrame::SetSpatialNavigationFindingFocusCandidateEnabled(
    bool spat_nav_finding_focus_candidate_enabled) {
  spat_nav_finding_focus_candidate_enabled_ = spat_nav_finding_focus_candidate_enabled;
}
...
std::unique_ptr<DragImage> LocalFrame::NodeImage(Node& node) {
  DraggedNodeImageBuilder image_node(*this, node, page_->AltDeviceScaleFactor());
...
std::unique_ptr<DragImage> LocalFrame::DragImageForSelection(float opacity) {
  ...
  return CreateDragImage(*this, opacity, kDoNotRespectImageOrientation,
                         painting_rect, page_->AltDeviceScaleFactor(), builder, PropertyTreeState::Root());
}
...
ClientRect* LocalFrame::GetSelectionBoundingRect() {
  return DomWindow()->getSelection()->GetBoundingRect();
}
...
inline LocalFrame::LocalFrame(LocalFrameClient* client,
    ...
    : Frame(client, page, owner, LocalWindowProxyManager::Create(*this)),
      ...
      spat_nav_finding_focus_candidate_enabled_(false),
