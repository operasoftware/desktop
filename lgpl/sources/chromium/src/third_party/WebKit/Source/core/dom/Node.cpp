...
bool Node::canStartSelection(SelectionStartPolicy selectionStartPolicy) const {
  ...
  return parent ? parent->canStartSelection(selectionStartPolicy) : true;
}
...
String Node::getPath() const {
  ...
  for (unsigned index = chain.size(); index > 0; --index) {
    ...
    if (node->isShadowRoot()) {
      int count = 0;
      for (ShadowRoot* shadowRoot = toShadowRoot(node)->olderShadowRoot();
           shadowRoot; shadowRoot = shadowRoot->olderShadowRoot())
        ++count;
      path.append(String::format("/#shadow-root[%d]", count));
      continue;
    }
        ...
        if (node->previousSibling() || node->nextSibling()) {
          ...
          if (!idAttr.isEmpty())
            path.append(String::format("[@id=\"%s\" and position()=%d]",
                                       idAttr.getString().utf8().data(),
                                       count));
          else
            path.append(String::format("[%d]", count));
        } else if (!idAttr.isEmpty()) {
          path.append(
              String::format("[@id=\"%s\"]", idAttr.getString().utf8().data()));
        }
        break;
      }
      case kTextNode:
