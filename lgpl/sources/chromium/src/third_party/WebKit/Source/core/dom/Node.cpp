...
bool Node::CanStartSelection(SelectionStartPolicy selection_start_policy) const {
  ...
  return parent ? parent->CanStartSelection(selection_start_policy) : true;
}
...
String Node::GetPath() const {
  ...
  for (unsigned index = chain.size(); index > 0; --index) {
    ...
    if (node->IsShadowRoot()) {
      int count = 0;
      for (ShadowRoot* shadow_root = ToShadowRoot(node)->OlderShadowRoot();
