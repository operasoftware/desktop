...
Document::Document(const DocumentInit& initializer,
    ...
    : ContainerNode(0, kCreateDocument),
      ...
      content_editables_controller_(this, nullptr),
...
FormController& Document::GetFormController() {
  if (!form_controller_) {
    ...
    if (history_item)
      history_item->SetFormState(FormElementsState());
  }
...
ContentEditablesController* Document::GetContentEditablesController() {
  if (!content_editables_controller_) {
    content_editables_controller_ = ContentEditablesController::Create();
    HistoryItem* history_item = Loader() ? Loader()->GetHistoryItem() : nullptr;
    if (history_item)
      history_item->SetContentEditablesState(GetContentEditableElementsState());
  }
  return content_editables_controller_.Get();
}

ContentEditablesState* Document::GetContentEditableElementsState() const {
  if (!content_editables_controller_)
    return nullptr;
  return content_editables_controller_->GetContentEditablesState();
}

void Document::SetStateForNewContentEditableElements(
    const Vector<String>& state_vector) {
  if (!state_vector.size())
    return;
  GetContentEditablesController()->SetContentEditablesContent(state_vector);
}
...
String Document::UserAgent(const KURL& url) const {
  return GetFrame() ? GetFrame()->Loader().UserAgent(url) : String();
}
...
const KURL Document::FirstPartyForCookies() const {
  ...
  SecurityOrigin* origin = top.GetSecurityContext()->GetSecurityOrigin();
  if (origin && SecurityPolicy::IsOriginFirstPartyWhenTopLevel(*origin))
    return top_document_url;
#endif  // OPERA_DESKTOP
...
Vector<IconURL> Document::IconURLs(int icon_types_mask) {
  ...
  for (HTMLLinkElement* link_element = first_element; link_element;
    ...
    } else if (link_element->GetIconType() == kPinnedIcon) {
      if (first_pinned_icon.icon_type_ != kInvalidIcon)
        secondary_icons.push_back(first_pinned_icon);
      first_pinned_icon = new_url;
      Color color = Color::kTransparent;
      if (CSSParser::ParseColor(
              color, link_element->Color().GetString().StripWhiteSpace(),
              true)) {
        first_pinned_icon.color_ = color;
      }
    } else {
      NOTREACHED();
    }
  ...
  if (first_pinned_icon.icon_type_ != kInvalidIcon)
    icon_urls.push_back(first_pinned_icon);
...
DEFINE_TRACE(Document) {
  ...
  visitor->Trace(content_editables_controller_);
