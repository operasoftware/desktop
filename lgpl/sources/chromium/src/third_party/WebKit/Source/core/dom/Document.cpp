...
Document::Document(const DocumentInit& initializer,
    ...
    : ContainerNode(0, CreateDocument),
      ...
      m_contentEditablesController(this, nullptr),
...
FormController& Document::formController() {
  if (!m_formController) {
    ...
    if (m_frame && m_frame->loader().currentItem() &&
      ...
      m_frame->loader().currentItem()->setFormState(formElementsState());
  }
...
ContentEditablesController* Document::contentEditablesController() {
  if (!m_contentEditablesController) {
    m_contentEditablesController = ContentEditablesController::create();
    if (m_frame && m_frame->loader().currentItem() &&
        m_frame->loader().currentItem()->isCurrentDocument(this))
      m_frame->loader().currentItem()->setContentEditablesState(
          contentEditableElementsState());
  }
  return m_contentEditablesController.get();
}

ContentEditablesState* Document::contentEditableElementsState() const {
  if (!m_contentEditablesController)
    return 0;
  return m_contentEditablesController->getContentEditablesState();
}
...
void Document::setStateForNewContentEditableElements(
  ...
  if (!stateVector.size())
    return;
  contentEditablesController()->setContentEditablesContent(stateVector);
}
...
Vector<IconURL> Document::iconURLs(int iconTypesMask) {
  ...
  for (HTMLLinkElement* linkElement = firstElement; linkElement;
    ...
    } else if (linkElement->getIconType() == PinnedIcon) {
      if (firstPinnedIcon.m_iconType != InvalidIcon)
        secondaryIcons.push_back(firstPinnedIcon);
      firstPinnedIcon = newURL;
      Color color = Color::transparent;
      if (CSSParser::parseColor(
              color, linkElement->color().getString().stripWhiteSpace(),
              true)) {
        firstPinnedIcon.m_color = color;
      }
    } else {
      NOTREACHED();
    }
  ...
  if (firstPinnedIcon.m_iconType != InvalidIcon)
    iconURLs.push_back(firstPinnedIcon);
...
DEFINE_TRACE(Document) {
  ...
  visitor->trace(m_contentEditablesController);
