...
TEST_F(NodeTest, canStartSelection) {
  ...
  const SelectionStartPolicy selectionStartPolicy =
      SelectionStartPolicy::Default;
  EXPECT_FALSE(one->canStartSelection(selectionStartPolicy));
  EXPECT_FALSE(one->firstChild()->canStartSelection(selectionStartPolicy));
  EXPECT_TRUE(two->canStartSelection(selectionStartPolicy));
  EXPECT_TRUE(two->firstChild()->canStartSelection(selectionStartPolicy));
}
