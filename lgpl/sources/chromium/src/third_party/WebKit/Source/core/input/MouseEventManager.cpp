...
WebInputEventResult MouseEventManager::handleMousePressEvent(
  ...
  m_mouseDownMayStartDrag = singleClick && !isExtendingSelection(event) &&
...
bool MouseEventManager::handleDrag(const MouseEventWithHitTestResults& event,
  ...
  auto& selectionController = m_frame->eventHandler().selectionController();

  if (!m_mouseDownMayStartDrag) {
    return initiator == DragInitiator::Mouse &&
           !selectionController.mouseDownMayStartSelect() &&
           !selectionController.linkSelectionMightStartDuringDrag() &&
           !m_mouseDownMayStartAutoscroll;
  }

  if (initiator == DragInitiator::Mouse) {
    const bool canStartLinkSelection =
        selectionController.linkSelectionMightStartDuringDrag();
    ThresholdAction result = getActionForDragThreshold(event.event().position(),
                                                       canStartLinkSelection);
    switch (result) {
      ...
      case ThresholdAction::LINK_SELECTION_CANCELED:
        ...
        selectionController.setLinkSelectionMightStartDuringDrag(false);
        return true;
...
ThresholdAction MouseEventManager::getActionForDragThreshold(
    const IntPoint& dragLocationInRootFrame,
    bool canStartLinkSelection) const {
  FrameView* view = m_frame->view();
  if (!view)
