...
static bool isContentEditableElement(HTMLElement* element) {
  String contentEditableString = element->contentEditable();
  return contentEditableString == "true" ||
         contentEditableString == "plaintext-only";
}
...
Node::InsertionNotificationRequest HTMLElement::insertedInto(
  ...
  if (isContentEditableElement(this) && insertionPoint->isConnected()) {
    document().contentEditablesController()->registerContentEditableElement(
        this);
  }
  return Element::insertedInto(insertionPoint);
}

void HTMLElement::removedFrom(ContainerNode* insertionPoint) {
  if (document().contentEditablesController()->isRegistered(this)) {
    document().contentEditablesController()->unregisterContentEditableElement(
        this);
  }
  Element::removedFrom(insertionPoint);
}

void HTMLElement::finishParsingChildren() {
  Element::finishParsingChildren();
  if (isContentEditableElement(this)) {
    document().contentEditablesController()->restoreContentsIn(this);
  }
}
