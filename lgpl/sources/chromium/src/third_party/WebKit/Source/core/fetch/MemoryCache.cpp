...
void MemoryCache::evictCompressedImages(int image_quality) {
  for (ResourceMapIndex::iterator mapiter = m_resourceMaps.begin();
       mapiter != m_resourceMaps.end(); ++mapiter) {
    HeapVector<Member<Resource>> evicting;

    ResourceMap* resourcemap = mapiter->value.get();
    for (ResourceMap::iterator i = resourcemap->begin();
         i != resourcemap->end(); ++i) {
      Resource* resource = i->value->resource();
      int used_image_quality;
      if (resource->response().isTurboCompressedImage(&used_image_quality) &&
          used_image_quality < image_quality)
        evicting.append(resource);
    }
