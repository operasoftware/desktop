...
TEST_F(EventHandlerTest, HitOnTextShowsIBeam) {
  ...
  HitTestResult hit =
      GetDocument().GetFrame()->GetEventHandler().HitTestResultAtPoint(
          location);
  EXPECT_TRUE(text->CanStartSelection(SelectionStartPolicy::kDefault));
  EXPECT_TRUE(
...
TEST_F(EventHandlerTest, HitOnUserSelectNoneDoesNotShowIBeam) {
  ...
  HitTestResult hit =
      GetDocument().GetFrame()->GetEventHandler().HitTestResultAtPoint(
          location);
  EXPECT_FALSE(text->CanStartSelection(SelectionStartPolicy::kDefault));
  EXPECT_FALSE(
      GetDocument().GetFrame()->GetEventHandler().ShouldShowIBeamForNode(text,
                                                                         hit));
}
...
TEST_F(EventHandlerTest, ImagesCannotStartSelection) {
  ...
  HitTestResult hit =
      GetDocument().GetFrame()->GetEventHandler().HitTestResultAtPoint(
          location);
  EXPECT_FALSE(img->CanStartSelection(SelectionStartPolicy::kDefault));
  EXPECT_FALSE(
