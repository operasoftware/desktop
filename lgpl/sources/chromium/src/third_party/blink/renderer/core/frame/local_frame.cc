...
void LocalFrame::SearchInnerText(const WTF::Vector<WTF::String>& words,
                                 SearchInnerTextCallback callback) {
  WTF::Vector<uint32_t> found;
  ...
  for (uint32_t index = 0; index < words.size(); ++index) {
    const WTF::String& word = words[index];
    DCHECK(!word.IsEmpty());
    if (frame_text.FindIgnoringCase(word) != kNotFound)
      found.push_back(index);
  }
  std::move(callback).Run(found);
}

void LocalFrame::GetInnerText(GetInnerTextCallback callback) {
  HTMLElement* body = GetDocument()->body();
  if (!body) {
