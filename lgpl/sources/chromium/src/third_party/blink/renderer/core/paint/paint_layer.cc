...
void PaintLayer::Destroy() {
  ...
  if (resource_info_) {
    ...
    if (style.HasFilter()) {
      ...
      style.Filter().RemoveGpuShaderClient(*resource_info_);
...
#if BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
  SkColor4f backdrop = SkColors::kTransparent;
  if (!filter.IsEmpty() && GetLayoutObject().BackgroundTransfersToView()) {
    backdrop = GetLayoutObject()
                   .GetFrameView()
                   ->DocumentBackgroundColor()
                   .toSkColor4f();
  }
#endif  // BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
  operations = FilterEffectBuilder(
                   reference_box, zoom,
