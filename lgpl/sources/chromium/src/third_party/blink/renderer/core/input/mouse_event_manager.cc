...
// Drag threshold for triggering selection on a link. Exceeding the threshold
// horizontally triggers link selection while exceeding it vertically allows
// link to be dragged when, and if, link's drag threshold is exceeded.
const int kLinkSelectionHysteresis = 3;
...
enum class ThresholdAction {
  ...
  LINK_SELECTION_STARTED,
  // Cancel further attemps to start link selection.
  LINK_SELECTION_CANCELED,
  // Start link drag.
  LINK_DRAG_STARTED,
  // No action triggered so far.
  NO_ACTION,
};
...
WebInputEventResult MouseEventManager::HandleMousePressEvent(
  ...
  // If |SelectionController| handles selection it calls |PositionForPoint()|
  // which requires |kPrePaintClean|.
  ...
  mouse_down_may_start_drag_ = single_click && !IsExtendingSelection(event) &&
                               (!IsSelectionOverLink(event) ||
                                frame_->GetEventHandler()
                                    .GetSelectionController()
                                    .LinkSelectionMightStartDuringDrag());
...
bool MouseEventManager::HandleDrag(const MouseEventWithHitTestResults& event,
  ...
  auto& selection_controller = frame_->GetEventHandler().GetSelectionController();

  if (!mouse_down_may_start_drag_) {
    return initiator == DragInitiator::kMouse &&
           !selection_controller.MouseDownMayStartSelect() &&
           !selection_controller.LinkSelectionMightStartDuringDrag() &&
           !mouse_down_may_start_autoscroll_;
  }

  if (initiator == DragInitiator::kMouse) {
    const bool can_start_link_selection =
        selection_controller.LinkSelectionMightStartDuringDrag();
    ThresholdAction result = GetActionForDragThreshold(
        gfx::ToFlooredPoint(event.Event().PositionInRootFrame()),
        can_start_link_selection);
    switch (result) {
      case ThresholdAction::LINK_SELECTION_STARTED:
        // Enough horizontal movement to enable link selection.
        InvalidateClick();
        return false;
      case ThresholdAction::LINK_SELECTION_CANCELED:
        // We've exceeded vertical threshold for link selection, so don't
        // attempt link selection in future drag events.
        selection_controller.SetLinkSelectionMightStartDuringDrag(false);
        return true;
      case ThresholdAction::LINK_DRAG_STARTED:
        // Start link drag.
        break;
      case ThresholdAction::NO_ACTION:
        // No drag or link selection triggered.
...
ThresholdAction MouseEventManager::GetActionForDragThreshold(
    const gfx::Point& drag_location_in_root_frame,
    bool can_start_link_selection) const {
  LocalFrameView* view = frame_->View();
  if (!view)
    return ThresholdAction::NO_ACTION;
  gfx::Point drag_location =
      view->ConvertFromRootFrame(drag_location_in_root_frame);
  gfx::Vector2d delta = drag_location - mouse_down_pos_;

  if (can_start_link_selection) {
    if (abs(delta.x()) >= kLinkSelectionHysteresis)
      return ThresholdAction::LINK_SELECTION_STARTED;

    if (abs(delta.y()) >= kLinkSelectionHysteresis)
      return ThresholdAction::LINK_SELECTION_CANCELED;
  }
  ...
  if (abs(delta.x()) >= kDragThresholdX || abs(delta.y()) >= kDragThresholdY) {
    return ThresholdAction::LINK_DRAG_STARTED;
  }
