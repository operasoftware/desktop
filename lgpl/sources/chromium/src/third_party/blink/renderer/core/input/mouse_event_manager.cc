...
ThresholdAction MouseEventManager::GetActionForDragThreshold(
    const IntPoint& drag_location_in_root_frame,
  ...
  if (can_start_link_selection) {
    if (abs(delta.Width()) >= kLinkSelectionHysteresis)
      return ThresholdAction::LINK_SELECTION_STARTED;

    if (abs(delta.Height()) >= kLinkSelectionHysteresis)
  ...
  if (abs(delta.Width()) >= kDragThresholdX ||
