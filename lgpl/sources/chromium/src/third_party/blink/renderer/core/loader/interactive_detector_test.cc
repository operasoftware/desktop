...
TEST_F(InteractiveDetectorTest, RecordInputDelayUKM) {
  ...
  auto entries =
      test_ukm_recorder.GetEntriesByName(ukm::builders::InputEvent::kEntryName);
  EXPECT_EQ(1ul, entries.size());
  auto* entry = entries[0];
  test_ukm_recorder.ExpectEntryMetric(
      entry, ukm::builders::InputEvent::kInteractiveTiming_InputDelayName,
      delay.InMilliseconds());
  test_ukm_recorder.ExpectEntryMetric(
      entry, ukm::builders::InputEvent::kInteractiveTiming_ProcessingTimeName,
