...
FormController& Document::GetFormController() {
  if (!form_controller_) {
    ...
    if (history_item)
      history_item->SetFormState(GetDocumentState());
  }
...
const KURL Document::SiteForCookies() const {
  ...
  if (SecurityPolicy::IsOriginFirstPartyWhenTopLevel(*origin)) {
    if (auto* top_local_frame = DynamicTo<LocalFrame>(&top))
      return top_local_frame->GetDocument()->Url();
    return KURL(NullURL(), origin->ToString());
  }
#endif  // OPERA_DESKTOP
