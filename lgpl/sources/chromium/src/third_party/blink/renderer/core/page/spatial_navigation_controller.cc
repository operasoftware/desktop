...
bool SpatialNavigationController::HandleArrowKeyboardEvent(
  ...
  if (frame->IsLocalFrame())
    local_frame = To<LocalFrame>(frame);
  SetSpatialNavigationFindingFocusCandidateEnabled(local_frame, true);
  auto result = Advance(direction);
  SetSpatialNavigationFindingFocusCandidateEnabled(local_frame, false);
  return result;
}
