...
class CORE_EXPORT StyleEngine final : public GarbageCollected<StyleEngine>,
 ...
 public:
  ...
  GpuShaderResource* CacheStyleShader(const String& url) {
    return style_shader_cache_.CacheStyleShader(url);
  }
#endif  // BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
...
#if BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
  // Cache for sharing StyleFetchedShader between CSSValues referencing the same
  // URL.
  StyleShaderCache style_shader_cache_;
#endif  // BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
};
