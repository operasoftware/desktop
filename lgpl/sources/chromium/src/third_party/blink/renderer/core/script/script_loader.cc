...
#if BUILDFLAG(OPERA_BLINK_FEATURE_SCRIPT_TRACKER)
  auto* script_tracker = context_window->GetFrame()->GetScriptTracker();
  if (auto* active_script_info = script_tracker->GetActiveScriptInfo())
    prepared_pending_script_->SetGeneratingScriptInfo(active_script_info);
#endif
