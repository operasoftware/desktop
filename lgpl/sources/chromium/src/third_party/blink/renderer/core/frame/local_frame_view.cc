...
static cc::LayerSelection ComputeLayerSelection_(LocalFrame& frame) {
  if (!frame.View() || frame.View()->ShouldThrottleRendering())
    return {};

  return ComputeLayerSelection(frame);
}
...
void LocalFrameView::UpdateCompositedSelectionIfNeeded() {
  ...
  if (local_frame) {
    const cc::LayerSelection& selection = ComputeLayerSelection_(*local_frame);
