...
void LocalFrameMojoHandler::SearchInnerText(
    const WTF::Vector<WTF::String>& words,
    SearchInnerTextCallback callback) {
  WTF::Vector<mojom::blink::SearchTextResultPtr> found;
  ...
  const auto frame_text = body->innerTextWithShadowDom();

  for (uint32_t word_index = 0; word_index < words.size(); ++word_index) {
    const WTF::String& word = words[word_index];
    DCHECK(!word.empty());
    int occurrences = 0;
    auto index = frame_text.FindIgnoringCase(word);
    while (index != kNotFound) {
      ++occurrences;
      const wtf_size_t starting_index = index + word.length();
      if (starting_index >= frame_text.length() - 1)
        break;
      index = frame_text.FindIgnoringCase(word, starting_index);
    }
    if (occurrences) {
      auto result = mojom::blink::SearchTextResult::New();
...
void LocalFrameMojoHandler::GetInnerText(GetInnerTextCallback callback) {
  HTMLElement* body = GetDocument()->body();
  if (!body) {
  ...
  const auto frame_text = body->innerTextWithShadowDom();
  std::move(callback).Run(frame_text);
}

void LocalFrameMojoHandler::GetImages(GetImagesCallback callback) {
  WTF::Vector<mojom::blink::ImageInfoPtr> result;
  ...
  for (unsigned i = 0; Element* curr = images->item(i); ++i) {
    ...
    auto image_info = mojom::blink::ImageInfo::New();
    image_info->url = KURL(image_element.currentSrc());
    image_info->width = image_element.naturalWidth();
    image_info->height = image_element.naturalHeight();
    result.push_back(std::move(image_info));
  }
