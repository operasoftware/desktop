  ...
  if (filter_type == CSSValueID::kDropShadow) {
    ...
    if (filter_type == CSSValueID::kBrightness) {
      ...
      parsed_value = ConsumeLength(args, context,
                                   CSSPrimitiveValue::ValueRange::kNonNegative);
#if BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
    } else if (filter_type == CSSValueID::kOperaShader) {
      AtomicString uri = ConsumeUrlAsStringView(args, context).ToAtomicString();
      if (uri.IsNull()) {
        parsed_value = MakeGarbageCollected<CSSShaderValue>();
      } else {
        CSSValueList* argv = CSSValueList::CreateSpaceSeparated();
        float frame_id = 0;
        // Parse the legacy iFrame uniform value
        if (!args.AtEnd() && args.Peek().GetType() == kNumberToken) {
          CSSPrimitiveValue* frame_id_value =
              ConsumeNumber(args, context, CSSPrimitiveValue::ValueRange::kAll);
          if (frame_id_value && frame_id_value->IsNumber()) {
            frame_id = frame_id_value->GetFloatValue();
          }
        }

        if (!args.AtEnd()) {
          if (args.Peek().FunctionId() != CSSValueID::kOperaArgs) {
            return nullptr;
          }

          CSSParserTokenRange uniform_args = ConsumeFunction(args);
          if (!uniform_args.AtEnd()) {
            do {
              // for -opera-args() allow numbers...
              CSSValue* value = ConsumeNumber(
                  uniform_args, context, CSSPrimitiveValue::ValueRange::kAll);
              if (!value) {
                // ... and colors
                value = ConsumeColor(uniform_args, context);
              }
              if (!value) {
                return nullptr;
              }
              argv->Append(*value);
            } while (!uniform_args.AtEnd());
          }
        }
        parsed_value = MakeGarbageCollected<CSSShaderValue>(
            uri, context.CompleteURL(uri), context.GetReferrer(), argv,
            frame_id);
      }
#endif  // BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
    } else {
...
CSSValue* ConsumeImage(
  ...
  if (range.Peek().GetType() == kFunctionToken) {
    ...
    if (IsUASheetBehavior(context.Mode())) {
      return ConsumeLightDark(ConsumeImageOrNone, range, context);
    }
  }
  return nullptr;
}
...
CSSValue* ConsumeFontStyle(CSSParserTokenRange& range,
  ...
  CSSPrimitiveValue* start_angle = ConsumeAngle(
      range, context, std::nullopt, kMinObliqueValue, kMaxObliqueValue);
  if (!start_angle) {
