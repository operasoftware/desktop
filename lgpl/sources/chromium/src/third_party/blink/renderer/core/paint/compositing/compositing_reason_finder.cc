...
#if BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
static CompositingReasons CompositingReasonsForFilters(
    const ComputedStyle& style) {
  CompositingReasons reasons = CompositingReason::kNone;

  auto filters = style.Filter();
  for (wtf_size_t i = 0; i < filters.size(); i++) {
    const auto* filter = filters.at(i);
    if (filter->GetType() == FilterOperation::OperationType::kGpuShader) {
      const auto* op = DynamicTo<GpuShaderFilterOperation>(filter);
      if (op->Resource() && op->Resource()->IsLoaded()) {
        auto* shader = op->Resource()->GetGpuShader();
        DCHECK(shader);
        if (shader->NeedsCompositingLayer())
          reasons |= CompositingReason::kWillChangeFilter;
      }
    }
  }
...
#if BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
  reasons |= CompositingReasonsForFilters(style);
#endif  // BUILDFLAG(OPERA_FEATURE_BLINK_GPU_SHADER_CSS_FILTER)
