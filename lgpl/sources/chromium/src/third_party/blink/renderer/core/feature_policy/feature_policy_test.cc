...
const char* const kValidFeaturePolicies[] = {
    "",      // An empty policy.
    " ",     // An empty policy.
    ";;",    // Empty policies.
    ",,",    // Empty policies.
...
const char* const kInvalidFeaturePolicies[] = {
    "badfeaturename",
    "badfeaturename 'self'",
    "1.0",
...
TEST_F(FeaturePolicyParserTest, ParseValidPolicy) {
  for (const char* policy_string : kValidFeaturePolicies) {
    PolicyParserMessageBuffer logger;
...
TEST_F(FeaturePolicyParserTest, ParseInvalidPolicy) {
  for (const char* policy_string : kInvalidFeaturePolicies) {
    PolicyParserMessageBuffer logger;
    FeaturePolicyParser::Parse(policy_string, origin_a_.get(), origin_b_.get(),
                               logger, test_feature_name_map);
