...
class TestTextInputHostWaiter : public mojom::blink::TextInputHost {
 public:
  TestTextInputHostWaiter(blink::BrowserInterfaceBrokerProxy& provider)
      : provider_(provider) {}

  ~TestTextInputHostWaiter() override {
    provider_.SetBinderForTesting(mojom::blink::TextInputHost::Name_, {});
  }
 ...
 private:
  blink::BrowserInterfaceBrokerProxy& provider_;
...
TEST_F(LocalFrameTest, CharacterIndexAtPointWithPinchZoom) {
  ...
  TestTextInputHostWaiter waiter(main_frame->GetBrowserInterfaceBroker());
  waiter.Init(run_loop.QuitClosure());
