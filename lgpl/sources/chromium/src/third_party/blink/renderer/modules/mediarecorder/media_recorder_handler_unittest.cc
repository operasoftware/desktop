...
bool VideoEncoderQueuesInput(const MediaRecorderTestParams& params) {
#if BUILDFLAG(USE_SYSTEM_PROPRIETARY_CODECS) && BUILDFLAG(IS_WIN)
  if (String(params.codecs).Contains("avc1") &&
...
TEST_P(MediaRecorderHandlerTest, EncodeVideoFrames) {
  ...
  if (VideoEncoderQueuesInput(GetParam())) {
    GTEST_SKIP() << "The test's use of Gmock and RunLoops assumes the video "
                    "encoder doesn't queue input";
  }
