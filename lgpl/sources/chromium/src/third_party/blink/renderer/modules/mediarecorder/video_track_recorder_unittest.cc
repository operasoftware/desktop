...
// Using RunClosure666 instead of RunClosure to avoid symbol collisions in jumbo
// builds.
ACTION_P(RunClosure666, closure) {
  closure.Run();
}
...
TEST_P(VideoTrackRecorderTest, VideoEncoding) {
  ...
  EXPECT_CALL(*this, OnEncodedVideo(_, _, _, _, true))
      ...
      .WillOnce(DoAll(SaveArg<1>(&third_frame_encoded_data),
                      ...
                      RunClosure666(run_loop.QuitClosure())));
...
TEST_P(VideoTrackRecorderTest, EncodeFrameWithPaddedCodedSize) {
  ...
  EXPECT_CALL(*this, OnEncodedVideo(_, _, _, _, true))
      .Times(1)
      .WillOnce(RunClosure666(run_loop.QuitClosure()));
...
TEST_F(VideoTrackRecorderTest, ReleasesFrame) {
  ...
  EXPECT_CALL(*this, OnEncodedVideo(_, _, _, _, true))
      .Times(1)
      .WillOnce(RunClosure666(run_loop.QuitWhenIdleClosure()));
  Encode(video_frame, base::TimeTicks::Now());
  video_frame = nullptr;
  run_loop.Run();
