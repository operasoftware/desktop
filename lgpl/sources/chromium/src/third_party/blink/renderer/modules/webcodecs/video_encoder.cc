...
bool MayHaveOSSoftwareEncoder(media::VideoCodecProfile profile) {
  ...
  // software encoder.
  // External OpenH264 is equivalent to OS software encoding here because it
  // also uses the VEA framework.
  //
...
VideoEncoder::CreateAcceleratedVideoEncoder(
  ...
  return std::make_unique<
      ...
      std::make_unique<media::VideoEncodeAcceleratorAdapter>(
          ...
          /*allow_multislice=*/true, required_encoder_type));
}
...
VideoEncoder::CreateExternalH264VideoEncoder(
  ...
  // Not really accelerated, but using the VEA framework to encode (in SW) in
  // another process.
  return CreateAcceleratedVideoEncoder(profile, options,
                                       external_software_factories,
                                       HardwarePreference::kNoPreference);
#else
  return nullptr;
...
std::unique_ptr<media::VideoEncoder> VideoEncoder::CreateSoftwareVideoEncoder(
  ...
  switch (codec) {
    ...
    case media::VideoCodec::kH264:
      ...
      if (result) {
        // Wrapping in OffloadingVideoEncoder is counterproductive in this case.
        return result;
      }
  ...
  if (config.hw_pref == HardwarePreference::kPreferHardware ||
...
#if BUILDFLAG(IS_MAC) && BUILDFLAG(ENABLE_EXTERNAL_OPENH264)
    std::unique_ptr<media::VideoEncoder> result;
#else
    ...
    if (config.hw_pref == HardwarePreference::kPreferHardware) {
      ...
      auto fallback_encoder = CreateSoftwareVideoEncoder(
          this, /*fallback=*/false, config.codec, config.profile, config.options,
          external_software_factories);
...
#if BUILDFLAG(ENABLE_EXTERNAL_OPENH264)
  RetrieveExternalSoftwareFactoriesWithKnownEncoderSupport(CrossThreadBindOnce(
...
void VideoEncoder::ContinueConfigureWithExternalSoftwareFactories(
  ...
  RetrieveGpuFactoriesWithKnownEncoderSupport(
      ...
      MakeUnwrappingCrossThreadWeakHandle(this), MakeUnwrappingCrossThreadHandle(request),
