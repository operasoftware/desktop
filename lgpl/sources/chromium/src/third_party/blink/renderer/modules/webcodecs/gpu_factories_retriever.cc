...
GetExternalSoftwareFactoriesOnMainThread() {
  DCHECK(IsMainThread());
  return Platform::Current()->GetExternalSoftwareFactories();
}

void RetrieveExternalSoftwareFactories(OutputCB result_callback) {
  if (IsMainThread()) {
    std::move(result_callback).Run(GetExternalSoftwareFactoriesOnMainThread());
    return;
  }
  ...
  Thread::MainThread()->GetDeprecatedTaskRunner()->PostTaskAndReplyWithResult(
      FROM_HERE,
      ConvertToBaseOnceCallback(
          CrossThreadBindOnce(&GetExternalSoftwareFactoriesOnMainThread)),
      ConvertToBaseOnceCallback(std::move(result_callback)));
}
...
void RetrieveExternalSoftwareFactoriesWithKnownEncoderSupport(
  ...
  RetrieveExternalSoftwareFactories(std::move(factories_callback));
}
#endif  // BUILDFLAG(ENABLE_EXTERNAL_OPENH264)
