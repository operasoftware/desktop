...
bool StatsCollectingDecoder::Configure(const Settings& settings) {
  ...
  bool ret = decoder_->Configure(settings);

  metrics_info_ = MetricsInfo{
      .codec = WebRtcToMediaVideoCodec(settings.codec_type()),
      .status = ret ? DecoderStatus::kOk : DecoderStatus::kConfigureError,
  };
...
int32_t StatsCollectingDecoder::Decode(const webrtc::EncodedImage& input_image,
  ...
  MaybeUpdateDecodeMetrics(ret, DecoderStatus::kDecodeError);
  return ret;
}
...
int32_t StatsCollectingDecoder::RegisterDecodeCompleteCallback(
  ...
  int32_t ret = decoder_->RegisterDecodeCompleteCallback(this);
  MaybeUpdateDecodeMetrics(ret, DecoderStatus::kRegisterCallbackError);
  return ret;
}
...
int32_t StatsCollectingDecoder::Release() {
  ...
  DVLOG(3) << __func__;
  const bool hw_accelerated = decoder_->GetDecoderInfo().is_hardware_accelerated;
  int32_t ret = decoder_->Release();

  MaybeUpdateDecodeMetrics(ret, DecoderStatus::kReleaseError);
  ReportDecodeMetrics(hw_accelerated);
...
void StatsCollectingDecoder::MaybeUpdateDecodeMetrics(int32_t result,
                                                      DecoderStatus status) {
  // We want to report the first error that occurs.
  if (metrics_info_.status == DecoderStatus::kOk &&
      result != WEBRTC_VIDEO_CODEC_OK) {
    metrics_info_.status = status;
  }
}

void StatsCollectingDecoder::ReportDecodeMetrics(bool hw_accelerated) {
  const auto histogram_name =
      base::StrCat({"Media.RTC.VideoDecoderStatus.",
                    media::GetCodecName(metrics_info_.codec),
                    hw_accelerated ? ".hw" : ".sw"});
  base::UmaHistogramEnumeration(histogram_name, metrics_info_.status);
}
