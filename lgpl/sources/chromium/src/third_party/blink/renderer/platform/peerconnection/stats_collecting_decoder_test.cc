...
class TestDecoderForMetrics final : public webrtc::VideoDecoder {
 public:
  ...
  explicit TestDecoderForMetrics(Acceleration initial_acceleration) {
    set_acceleration(initial_acceleration);
  }

  void set_status(int32_t status) { status_ = status; }

  // Will perform HW -> SW fallback on the next call to Configure() or
  // Decode().
  void ScheduleFallback() {
    CHECK(!pending_acceleration_);
    CHECK(decoder_info_.is_hardware_accelerated);
  ...
  bool Configure(const Settings& settings) override {
    ...
    return status_ == WEBRTC_VIDEO_CODEC_OK;
  }
  int32_t Decode(const webrtc::EncodedImage& input_image,
                 bool missing_frames,
                 int64_t render_time_ms) override {
...
class StatsCollectingDecoderMetricsTest : public ::testing::Test {
 public:
   // TestDecoderForMetrics is owned by the returned StatsCollectingDecoder.
  std::pair<std::unique_ptr<StatsCollectingDecoder>,
            base::raw_ptr<TestDecoderForMetrics>>
  CreateMetricsDecoder(const std::string& codec_name,
                       TestDecoderForMetrics::Acceleration acceleration) {
    auto test_decoder = std::make_unique<TestDecoderForMetrics>(acceleration);
    auto* test_decoder_raw = test_decoder.get();
    auto metrics_decoder = std::make_unique<StatsCollectingDecoder>(
        webrtc::SdpVideoFormat{codec_name}, std::move(test_decoder),
        base::DoNothing());
    return {std::move(metrics_decoder), test_decoder_raw};
  }
...
TEST_F(StatsCollectingDecoderMetricsTest, InitAndRelease_H264) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kUnaccelerated);

  webrtc::VideoDecoder::Settings codec_settings;
  codec_settings.set_codec_type(webrtc::VideoCodecType::kVideoCodecH264);
  metrics_decoder->Configure(codec_settings);
  metrics_decoder->Release();

  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoDecoderStatus.h264.sw",
      StatsCollectingDecoder::DecoderStatus::kOk, 1);
}

TEST_F(StatsCollectingDecoderMetricsTest, InitAndRelease_H264_HW) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kHwAccelerated);
  ...
  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoDecoderStatus.h264.hw",
      StatsCollectingDecoder::DecoderStatus::kOk, 1);
}

TEST_F(StatsCollectingDecoderMetricsTest, InitAndRelease_H264_HW_SW) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kHwAccelerated);

  // The decoder will perform a HW -> SW fallback as part of Configure().
  wrapped_decoder->ScheduleFallback();
...
TEST_F(StatsCollectingDecoderMetricsTest, InitAndRelease_AV1) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "AV1", TestDecoderForMetrics::Acceleration::kUnaccelerated);

  webrtc::VideoDecoder::Settings codec_settings;
  codec_settings.set_codec_type(webrtc::VideoCodecType::kVideoCodecAV1);
  metrics_decoder->Configure(codec_settings);
  metrics_decoder->Release();

  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoDecoderStatus.av1.sw",
      StatsCollectingDecoder::DecoderStatus::kOk, 1);
}

TEST_F(StatsCollectingDecoderMetricsTest, InitAndRelease_InitError) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kUnaccelerated);

  webrtc::VideoDecoder::Settings codec_settings;
  codec_settings.set_codec_type(webrtc::VideoCodecType::kVideoCodecH264);
  wrapped_decoder->set_status(WEBRTC_VIDEO_CODEC_UNINITIALIZED);
  metrics_decoder->Configure(codec_settings);

  wrapped_decoder->set_status(WEBRTC_VIDEO_CODEC_OK);
  metrics_decoder->Release();

  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoDecoderStatus.h264.sw",
      StatsCollectingDecoder::DecoderStatus::kConfigureError, 1);
}

TEST_F(StatsCollectingDecoderMetricsTest, InitDecodeAndRelease) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kUnaccelerated);
  ...
  metrics_decoder->Decode({}, false, 90000);
...
TEST_F(StatsCollectingDecoderMetricsTest, InitDecodeAndRelease_HW_SW) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kHwAccelerated);
  ...
  // The decoder will perform a HW -> SW fallback as part of Decode().
  wrapped_decoder->ScheduleFallback();
...
TEST_F(StatsCollectingDecoderMetricsTest,
       InitDecodeAndRelease_DecodeError_HW_SW) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kHwAccelerated);
  ...
  // The decoder will perform a HW -> SW fallback as part of Decode(), but will
  // fail eventually, anyway.
  wrapped_decoder->ScheduleFallback();
  wrapped_decoder->set_status(WEBRTC_VIDEO_CODEC_ERROR);
  metrics_decoder->Decode({}, false, 90001);
  ...
  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoDecoderStatus.h264.sw",
      StatsCollectingDecoder::DecoderStatus::kDecodeError, 1);
}

TEST_F(StatsCollectingDecoderMetricsTest, InitDecodeAndRelease_ReleaseError) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kUnaccelerated);
  ...
  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoDecoderStatus.h264.sw",
      StatsCollectingDecoder::DecoderStatus::kReleaseError, 1);
}

TEST_F(StatsCollectingDecoderMetricsTest,
       InitDecodeAndRelease_DecodeAndReleaseError) {
  auto [metrics_decoder, wrapped_decoder] = CreateMetricsDecoder(
      "H264", TestDecoderForMetrics::Acceleration::kUnaccelerated);
  ...
  // The second Decode() fails.
  metrics_decoder->Decode({}, false, 90000);

  wrapped_decoder->set_status(WEBRTC_VIDEO_CODEC_ERROR);
  metrics_decoder->Decode({}, false, 90001);

  // Release() fails too, but Decode() failed first, so we record that one.
  metrics_decoder->Release();
