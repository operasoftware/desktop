...
class PLATFORM_EXPORT StatsCollectingEncoder
 ...
 public:
  // Recorded as metrics data. Don't remove or rearrange values.
  enum class EncoderStatus {
    kOk = 0,
    kInitError = 1,
    kEncodeError = 2,
    kRegisterCallbackError = 3,
    kReleaseError = 4,
    kMaxValue = kReleaseError,
  };
 ...
 private:
  ...
  struct MetricsInfo {
    ...
    EncoderStatus status = EncoderStatus::kOk;
  };
  ...
  void MaybeUpdateEncodeMetrics(int32_t result, EncoderStatus status);
  void ReportEncodeMetrics(bool hw_accelerated);
