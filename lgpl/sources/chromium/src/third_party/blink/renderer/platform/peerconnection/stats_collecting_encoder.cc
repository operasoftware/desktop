...
int StatsCollectingEncoder::InitEncode(
  ...
  metrics_info_ = MetricsInfo{
      .codec = WebRtcToMediaVideoCodec(codec_settings->codecType),
      .status = ret == WEBRTC_VIDEO_CODEC_OK ? EncoderStatus::kOk
                                             : EncoderStatus::kInitError,
  };
...
int32_t StatsCollectingEncoder::RegisterEncodeCompleteCallback(
  ...
  int32_t ret = encoder_->RegisterEncodeCompleteCallback(this);
  MaybeUpdateEncodeMetrics(ret, EncoderStatus::kRegisterCallbackError);
  return ret;
}
...
int32_t StatsCollectingEncoder::Release() {
  ...
  DVLOG(3) << __func__;
  const bool hw_accelerated = encoder_->GetEncoderInfo().is_hardware_accelerated;
  int32_t ret = encoder_->Release();

  MaybeUpdateEncodeMetrics(ret, EncoderStatus::kReleaseError);
  ReportEncodeMetrics(hw_accelerated);
...
int32_t StatsCollectingEncoder::Encode(
  ...
  MaybeUpdateEncodeMetrics(ret, EncoderStatus::kEncodeError);
  return ret;
}
...
void StatsCollectingEncoder::MaybeUpdateEncodeMetrics(int32_t result,
                                                      EncoderStatus status) {
  // We want to report the first error that occurs.
  if (metrics_info_.status == EncoderStatus::kOk &&
      result != WEBRTC_VIDEO_CODEC_OK) {
    metrics_info_.status = status;
  }
}

void StatsCollectingEncoder::ReportEncodeMetrics(bool hw_accelerated) {
  const auto histogram_name =
      base::StrCat({"Media.RTC.VideoEncoderStatus.",
