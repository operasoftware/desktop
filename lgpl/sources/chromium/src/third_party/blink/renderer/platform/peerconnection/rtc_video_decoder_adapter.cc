...
std::unique_ptr<RTCVideoDecoderAdapter> RTCVideoDecoderAdapter::CreateInternal(
  ...
  if (gpu_factories) {
    ...
    rtc_video_decoder_adapter = base::WrapUnique(new RTCVideoDecoderAdapter(
        std::move(decoder), media_task_runner, config));
    if (rtc_video_decoder_adapter->InitializeSync(config))
      return rtc_video_decoder_adapter;
    // Initialization failed - post delete task and try next supported
    // implementation, if any.
...
RTCVideoDecoderAdapter::RTCVideoDecoderAdapter(
    ...
    scoped_refptr<base::SequencedTaskRunner> media_task_runner,
    const media::VideoDecoderConfig& config)
    : RTCVideoDecoderAdapter(nullptr, config) {
  DCHECK(decoder);
  media_task_runner_ = std::move(media_task_runner);
  video_decoder_ = std::move(decoder);
  decoder_info_.is_hardware_accelerated =
      IsHardwareAcceleratedDecoder(*video_decoder_);
}
...
bool RTCVideoDecoderAdapter::HasSoftwareFallback(
