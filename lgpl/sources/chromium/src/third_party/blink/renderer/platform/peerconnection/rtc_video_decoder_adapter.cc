...
#elif BUILDFLAG(USE_SYSTEM_PROPRIETARY_CODECS) || \
    BUILDFLAG(ENABLE_EXTERNAL_OPENH264)
  return is_platform_decoder;
#else
  return true;
#endif
}
...
RTCVideoDecoderAdapter::Impl::EnqueueBuffer(
  ...
  if (HasSoftwareFallback(video_codec_, video_decoder_->IsPlatformDecoder()) &&
...
std::unique_ptr<RTCVideoDecoderAdapter> RTCVideoDecoderAdapter::Create(
  ...
  return CreateInternal(gpu_factories, nullptr, nullptr, format,
                        std::move(resolution_monitor));
}
...
std::unique_ptr<RTCVideoDecoderAdapter> RTCVideoDecoderAdapter::Create(
  ...
  return CreateInternal(nullptr, std::move(decoder),
                        std::move(media_task_runner), format,
                        std::move(resolution_monitor));
}

std::unique_ptr<RTCVideoDecoderAdapter> RTCVideoDecoderAdapter::CreateInternal(
    media::GpuVideoAcceleratorFactories* gpu_factories,
    std::unique_ptr<media::VideoDecoder> decoder,
    scoped_refptr<base::SequencedTaskRunner> media_task_runner,
    const webrtc::SdpVideoFormat& format,
  ...
  if (WebRtcToMediaVideoCodec(video_codec_type) ==
  ...
  if (gpu_factories) {
    ...
    rtc_video_decoder_adapter = base::WrapUnique(new RTCVideoDecoderAdapter(
        gpu_factories, nullptr, nullptr, config, std::move(resolution_monitor)));
    }
  } else {
    rtc_video_decoder_adapter = base::WrapUnique(new RTCVideoDecoderAdapter(
        nullptr, std::move(decoder), std::move(media_task_runner),
        config, std::move(resolution_monitor)));
  }
...
RTCVideoDecoderAdapter::RTCVideoDecoderAdapter(
    ...
    const media::VideoDecoderConfig& config,
    std::unique_ptr<ResolutionMonitor> resolution_monitor)
    : media_task_runner_(gpu_factories ? gpu_factories->GetTaskRunner()
  ...
  if (PostCrossThreadTask(
          *media_task_runner_.get(), FROM_HERE,
          CrossThreadBindOnce(&RTCVideoDecoderAdapter::Impl::Initialize,
                              ...
                              CrossThreadUnretained(&is_platform_decoder_)))) {
...
RTCVideoDecoderAdapter::DecodeInternal(const webrtc::EncodedImage& input_image,
  ...
  CHECK(buffer);
  if (HasSoftwareFallback(config_.codec(), is_platform_decoder_) &&
...
bool RTCVideoDecoderAdapter::CheckResolutionAndNumInstances(
    const media::DecoderBuffer& buffer) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
  DCHECK(HasSoftwareFallback(config_.codec(), is_platform_decoder_));
