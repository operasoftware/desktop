...
void WebMediaPlayerImpl::SetVolume(double volume) {
  ...
  client_->DidPlayerVolumeChange(
      HasAudio() ? volume : std::numeric_limits<double>::quiet_NaN());
...
void WebMediaPlayerImpl::OnError(media::PipelineStatus status) {
  ...
  ReportPipelineError(load_type_, pipeline_metadata_, status);
  simple_watch_timer_.Stop();
...
void WebMediaPlayerImpl::OnMetadata(const media::PipelineMetadata& metadata) {
  ...
  if (!HasVideo() && !HasAudio()) {
    ...
    media::ReportDemuxerErrorDetails(media::DemuxerErrorDetails::kNoTracks);
...
void WebMediaPlayerImpl::StartPipeline() {
  ...
  // The boolean value reported is irrelevant, we're only interested in the
  // total number of pipeline start attempts.
  switch (load_type_) {
    case kLoadTypeURL:
      UMA_HISTOGRAM_BOOLEAN("Opera.DSK.Media.SRC.Pipeline.Start", true);
      break;
    case kLoadTypeMediaSource:
      UMA_HISTOGRAM_BOOLEAN("Opera.DSK.Media.MSE.Pipeline.Start", true);
      break;
    default:
      NOTREACHED();
  }
...
void WebMediaPlayerImpl::ReportDryRunResult(const std::string& codec_name,
                                            bool result) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  media_metrics_provider_->SetDryRunResult(codec_name, result);
}
