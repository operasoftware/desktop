...
TEST_F(StatsCollectingEncoderTest, MethodCallsForwardedToInternalEncoder) {
  ...
  webrtc::VideoCodec* codec_settings_ptr = &codec_settings;
...
class TestEncoderForMetrics final : public webrtc::VideoEncoder {
 public:
  ...
  explicit TestEncoderForMetrics(Acceleration initial_acceleration) {
    set_acceleration(initial_acceleration);
  }

  void set_status(int32_t status) { status_ = status; }

  // Will perform HW -> SW fallback on the next call to InitEncode() or
  // Encode().
  void ScheduleFallback() {
    CHECK(!pending_acceleration_);
    CHECK(encoder_info_.is_hardware_accelerated);
    pending_acceleration_ = Acceleration::kUnaccelerated;
  }

  // webrtc::VideoEncoder:
  int InitEncode(const webrtc::VideoCodec* codec_settings,
                 const VideoEncoder::Settings& settings) override {
    MaybePerformFallback();
    return status_;
  }
  int32_t RegisterEncodeCompleteCallback(
      webrtc::EncodedImageCallback* callback) override {
  ...
  int32_t Release() override { return status_; }
  int32_t Encode(
      const webrtc::VideoFrame& frame,
      const std::vector<webrtc::VideoFrameType>* frame_types) override {
    MaybePerformFallback();
  ...
  EncoderInfo GetEncoderInfo() const override { return encoder_info_; }

 private:
  void set_acceleration(Acceleration acceleration) {
    encoder_info_.is_hardware_accelerated =
        acceleration == Acceleration::kHwAccelerated;
  }

  void MaybePerformFallback() {
    if (pending_acceleration_) {
      set_acceleration(*pending_acceleration_);
      pending_acceleration_.reset();
    }
  }

  EncoderInfo encoder_info_;
  int32_t status_ = WEBRTC_VIDEO_CODEC_OK;
  absl::optional<Acceleration> pending_acceleration_;
};

class StatsCollectingEncoderMetricsTest : public ::testing::Test {
 public:
   // TestEncoderForMetrics is owned by the returned StatsCollectingEncoder.
  std::pair<std::unique_ptr<StatsCollectingEncoder>,
            base::raw_ptr<TestEncoderForMetrics>>
  CreateMetricsEncoder(const std::string& codec_name,
                       TestEncoderForMetrics::Acceleration acceleration) {
    auto test_encoder = std::make_unique<TestEncoderForMetrics>(acceleration);
    auto* test_encoder_raw = test_encoder.get();
    auto metrics_encoder = std::make_unique<StatsCollectingEncoder>(
        webrtc::SdpVideoFormat{codec_name}, std::move(test_encoder),
        base::DoNothing());
    return {std::move(metrics_encoder), test_encoder_raw};
  }
...
TEST_F(StatsCollectingEncoderMetricsTest, InitAndRelease_H264) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kUnaccelerated);

  webrtc::VideoCodec codec_settings;
  codec_settings.codecType = webrtc::VideoCodecType::kVideoCodecH264;
  metrics_encoder->InitEncode(&codec_settings, kVideoEncoderSettings);
  metrics_encoder->Release();

  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoEncoderStatus.h264.sw",
      StatsCollectingEncoder::EncoderStatus::kOk, 1);
}

TEST_F(StatsCollectingEncoderMetricsTest, InitAndRelease_H264_HW) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kHwAccelerated);
  ...
  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoEncoderStatus.h264.hw",
      StatsCollectingEncoder::EncoderStatus::kOk, 1);
}

TEST_F(StatsCollectingEncoderMetricsTest, InitAndRelease_H264_HW_SW) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kHwAccelerated);

  // The encoder will perform a HW -> SW fallback as part of InitEncode().
  wrapped_encoder->ScheduleFallback();
...
TEST_F(StatsCollectingEncoderMetricsTest, InitAndRelease_AV1) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "AV1", TestEncoderForMetrics::Acceleration::kUnaccelerated);

  webrtc::VideoCodec codec_settings;
  codec_settings.codecType = webrtc::VideoCodecType::kVideoCodecAV1;
  metrics_encoder->InitEncode(&codec_settings, kVideoEncoderSettings);
  metrics_encoder->Release();

  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoEncoderStatus.av1.sw",
      StatsCollectingEncoder::EncoderStatus::kOk, 1);
}

TEST_F(StatsCollectingEncoderMetricsTest, InitAndRelease_InitError) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kUnaccelerated);

  webrtc::VideoCodec codec_settings;
  codec_settings.codecType = webrtc::VideoCodecType::kVideoCodecH264;
  wrapped_encoder->set_status(WEBRTC_VIDEO_CODEC_UNINITIALIZED);
  metrics_encoder->InitEncode(&codec_settings, kVideoEncoderSettings);

  wrapped_encoder->set_status(WEBRTC_VIDEO_CODEC_OK);
  metrics_encoder->Release();

  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoEncoderStatus.h264.sw",
      StatsCollectingEncoder::EncoderStatus::kInitError, 1);
}

TEST_F(StatsCollectingEncoderMetricsTest, InitEncodeAndRelease) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kUnaccelerated);
  ...
  const std::vector<webrtc::VideoFrameType> frame_types{
      webrtc::VideoFrameType::kVideoFrameKey};
  metrics_encoder->Encode(CreateMockFrame(320, 240, 90000), &frame_types);
...
TEST_F(StatsCollectingEncoderMetricsTest, InitEncodeAndRelease_HW_SW) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kHwAccelerated);
  ...
  // The encoder will perform a HW -> SW fallback as part of Encode().
  wrapped_encoder->ScheduleFallback();
...
TEST_F(StatsCollectingEncoderMetricsTest,
       InitEncodeAndRelease_EncodeError_HW_SW) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kHwAccelerated);
  ...
  // The encoder will perform a HW -> SW fallback as part of Encode(), but will
  // fail eventually, anyway.
  wrapped_encoder->ScheduleFallback();
  wrapped_encoder->set_status(WEBRTC_VIDEO_CODEC_ENCODER_FAILURE);
  metrics_encoder->Encode(CreateMockFrame(320, 240, 90001), &frame_types);
  ...
  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoEncoderStatus.h264.sw",
      StatsCollectingEncoder::EncoderStatus::kEncodeError, 1);
}

TEST_F(StatsCollectingEncoderMetricsTest, InitEncodeAndRelease_ReleaseError) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kUnaccelerated);
  ...
  histogram_tester_.ExpectUniqueSample(
      "Media.RTC.VideoEncoderStatus.h264.sw",
      StatsCollectingEncoder::EncoderStatus::kReleaseError, 1);
}

TEST_F(StatsCollectingEncoderMetricsTest,
       InitEncodeAndRelease_EncodeAndReleaseError) {
  auto [metrics_encoder, wrapped_encoder] = CreateMetricsEncoder(
      "H264", TestEncoderForMetrics::Acceleration::kUnaccelerated);
  ...
  // Release() fails too, but Encode() failed first, so we record that one.
  metrics_encoder->Release();
