...
AffixMgr::AffixMgr(const std::string& data, HashMgr** ptr, int * md)
{
#else
AffixMgr::AffixMgr(const char * affpath, HashMgr** ptr, int * md, const char * key) 
{
  ...
  if (parse_file(data)) {
    HUNSPELL_WARNING(stderr, "Failure loading aff data\n");
  }
#else
...
#ifdef HUNSPELL_OPERA_CLIENT
int  AffixMgr::parse_file(const std::string& data)
#else
...
#endif
{
  ...
  if (!afflst) {
    HUNSPELL_WARNING(stderr, "error: could not open affix description data\n");
    return 1;
  }
#else
  // open the affix file
