...
HashMgr::HashMgr(const std::string& aff_data,
    const std::string& dic_data)
{
#else
HashMgr::HashMgr(const char * tpath, const char * apath, const char * key)
{
  ...
  int ec = LoadAFLines();
#elif HUNSPELL_OPERA_CLIENT
  int ec = 0;
  load_config(aff_data);
  ec = load_tables(dic_data);
#else
...
int HashMgr::load_tables(const std::string& data)
#else
...
#ifdef HUNSPELL_OPERA_CLIENT
    HUNSPELL_WARNING(stderr, "error: empty dic data\n");
#else
...
#ifdef HUNSPELL_OPERA_CLIENT
int  HashMgr::load_config(const std::string& data)
#else
int  HashMgr::load_config(const char * affpath, const char * key)
#endif  // HUNSPELL_OPERA_CLIENT
{
...
#elif defined HUNSPELL_OPERA_CLIENT
  FileMgr * afflst = new FileMgr(data);
  if (!afflst) {
    HUNSPELL_WARNING(stderr, "No affix description data\n");
    return 1;
  }
#else
