...
#include "hunspell.h"
#if !defined HUNSPELL_CHROME_CLIENT && !defined HUNSPELL_OPERA_CLIENT
...
#endif
{
    ...
    enum InitStateBits
    {
      ...
      HAS_AFF = 1,
      HAS_DICT = 2,
      HAS_ALL = HAS_AFF | HAS_DICT
    };
    int init_mask = HAS_NONE;
    if (bdict_data && *bdict_data) {
      std::string dict_data =
          std::string(reinterpret_cast<const char*>(bdict_data), bdict_length);
      if (reader.OpenFromString(dict_data))
      {
        while (reader.HasMore())
        {
          if (reader.OpenCurrentEntryInZip())
          {
            const size_t max_read_bytes = 0x1000000;  // 16MiB
            if (reader.current_entry_info()->file_path().MatchesExtension(
                    FILE_PATH_LITERAL(".aff"))) {
              if (reader.ExtractCurrentEntryToString(max_read_bytes, &aff_data))
                init_mask |= HAS_AFF;
            } else if (reader.current_entry_info()->file_path().
                           MatchesExtension(FILE_PATH_LITERAL(".dic"))) {
              if (reader.ExtractCurrentEntryToString(max_read_bytes, &dic_data))
                init_mask |= HAS_DICT;
            }
          }
    ...
    inited_with_success = init_mask == HAS_ALL;
    if (!inited_with_success)
    {
      aff_data.clear();
      dic_data.clear();
    }
    pHMgr[0] = new HashMgr(aff_data, dic_data);
    if (pHMgr[0]) maxdic = 1;

    pAMgr = new AffixMgr(aff_data, pHMgr, &maxdic);
#else
